    ...
    partial class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    List<double> allKNETData = new();
                    List<double> allConfluentData = new();
                    for (int length = MinPacketLength; length <= MaxPacketLength; length *= PacketLengthMultiplier)
                    {
                        ...
                        // Write one row per (packets, length) after ALL repetitions complete.
                        // KNETData now contains Repeat × packets individual roundtrip
                        // measurements — one AppendLine per (packets, length), not per testIndex.
                        if (KNETData.Any() && ConfluentData.Any())
                        {
