    ...
    public class KNetSourceTestTask : KNetSourceTask<KNetSourceTestTask>
    {
        ...
        public override System.Collections.Generic.IList<SourceRecord> Poll()
        {
            ...
            for (long i = lineOffset; i < lines.LongLength; i++)
            {
                if (useSourceOffset)
                {
                    var record = CreateRecord(PartitionOrOffsetForKey((Java.Lang.String)FILENAME_FIELD, _filename), PartitionOrOffsetForKey((Java.Lang.String)POSITION_FIELD, i), _topic, Schema.STRING_SCHEMA, lines[i], DateTime.Now);
                    records.Add(record);
                }
                else
                {
