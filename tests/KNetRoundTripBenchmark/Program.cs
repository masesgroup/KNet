            ...
            try
            {
                ...
                for (int length = MinPacketLength; length <= MaxPacketLength; length *= PacketLengthMultiplier)
                {
                    ...
                    try
                    {
                        ...
                        var KNETProdSW = RoundRobinKNet(topicNameKNet, length, PacketToExchange, CheckOnConsume ? data : null);
                ...
                File.WriteAllText(Path.Combine(ResultsPath, $"roundrobin_results_{DateTime.Now:yyyyMMdd_HHmmss}.csv"), sb.ToString());
            }
