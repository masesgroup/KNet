...
namespace MASES.KNet.Consumer
{
    ...
    public class ConsumerRecords<K, V> : IEnumerable<ConsumerRecord<K, V>>, IAsyncEnumerable<ConsumerRecord<K, V>>
    {
        ...
        internal ConsumerRecords(Org.Apache.Kafka.Clients.Consumer.ConsumerRecords<byte[], byte[]> records, ISerDes<K> keyDeserializer, ISerDes<V> valueDeserializer)
        {
            _records = records;
...
#endif
        /// <summary>
        /// <see langword="true"/> if the <see cref="ConsumerRecords{K, V}"/> is empty
        /// </summary>
        public bool IsEmpty => _records.IsEmpty();
        /// <summary>
        /// The number of elements in <see cref="ConsumerRecords{K, V}"/>
        /// </summary>
        ...
        public ConsumerRecords<K, V> ApplyPrefetch(bool enablePrefetch = true, int prefetchThreshold = 10)
        {
...
#endif
        IEnumerator<ConsumerRecord<K, V>> IEnumerable<ConsumerRecord<K, V>>.GetEnumerator()
        {
#if NET7_0_OR_GREATER
            if (UsePrefetch())
                return new ConsumerRecordsPrefetchableEnumerator<K, V>(_records.Iterator(), _keyDeserializer, _valueDeserializer, false);
            else
#endif
                return new ConsumerRecordsEnumerator<K, V>(_records, _keyDeserializer, _valueDeserializer);
        }
        ...
        IAsyncEnumerator<ConsumerRecord<K, V>> IAsyncEnumerable<ConsumerRecord<K, V>>.GetAsyncEnumerator(CancellationToken cancellationToken)
        {
#if NET7_0_OR_GREATER
            if (UsePrefetch())
                return new ConsumerRecordsPrefetchableEnumerator<K, V>(_records.Iterator(), _keyDeserializer, _valueDeserializer, true, cancellationToken);
            else
#endif
                return new ConsumerRecordsEnumerator<K, V>(_records, _keyDeserializer, _valueDeserializer, cancellationToken);
        }
    }
}
