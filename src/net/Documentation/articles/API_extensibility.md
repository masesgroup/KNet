...
This is a void method, using **IExecute** the user of the library can invoke the method on the class. Anyway the developer can invoke the internal method directly from the instance of the object:
```C#
...
Now consider that the returned data type (`Future<RecordMetadata>`) was not yet implemented; a solution on this problem is to use directly the `send` Java method like in the following code snippet:
```C#
KafkaProducer producer = new KafkaProducer(...);
...
The example above consider that even the class `ProducerRecord` not implemented yet. Anyway it exists in JVM.
Each object, like `KafkaProducer` instance, exposes (hidded in the editor) two properties:
...
If no classes are available the solution comes from the global accessor available in JCOBridge and the code snippet is like the following:
...
The example above consider that even the class `KafkaProducer` not implemented yet. Anyway it exists in JVM.
In previous chapter the tutorial reports about two hidden properties in each object; these properties are just a comodoty reference to the real one available in `JCOBridge.Global`:
* **JVM** which access the JVM using methods;
* **DynJVM** which access the JVM using the Dynamic engine.
...
As exposed before each object, like `KafkaConsumer` instance, exposes (hidded in the editor) two properties.

Explaining the code:
* The first line creates a JVM object in C# style: `KafkaConsumer` lives in the CLR and has its countpart in the JVM.
...
**This callback feature needs a concrete class in the JVM and if it does not exist there is no way to use it from KafkaBridge.**
