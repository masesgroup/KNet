...
{
    [Cmdlet(VerbsCommon.New, "ProducerRecord")]
    [OutputType(typeof(ProducerRecord<,>))]
    public class NewProducerRecordCmdletCommand : KNetPSCmdlet
    {
        ...
        [Parameter(
            ...
            ValueFromPipeline = true,
            ValueFromPipelineByPropertyName = true,
            HelpMessage = "The topic name")]
        public string Topic { get; set; }

        // This method gets called once for each cmdlet in the pipeline when the pipeline starts executing
        protected override void BeginProcessing()
        {
            WriteVerbose("Begin ProducerRecord!");
        }
        ...
        protected override void ProcessCommand()
        {
            ...
            object key = Key;
            if (key is PSObject pskey) key = pskey.BaseObject;

            object value = Value;
            if (value is PSObject psvalue) value = psvalue.BaseObject;

            var recordType = typeof(ProducerRecord<,>).MakeGenericType(keyType, valueType);

            var record = Activator.CreateInstance(recordType, Topic, key, value);
